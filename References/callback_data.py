  ...
  """
	...
	if request.method == 'POST':
		...
		if request_user == http_user and request_passwd == http_passwd:
			...
			message_id = publisher.publish(topic_name, request_json.encode('utf-8'), ts=event_time.encode('utf-8'))
			...
			if 'ack' in request_dict:
				if (request_dict['ack'] == 'true' and request_dict['device'] and
					...
					if not e:
						...
						print('Datastore config: {} wrong length for deviceType: {}. Should be max 16 HEX characters (8 bytes)'.format(e[ds_property], device_type))
						return '', 204
					else:
